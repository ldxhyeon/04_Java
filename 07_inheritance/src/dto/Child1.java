...
public class Child1 extends Parent{
	 ...
	 * - super 뜻 : 상위의, 위쪽의
	 * 
	 * -> 현재 클래스의 객체의 상위 객체(부모) 생성자를 호출하는 구문
	 * 
	...
	public void print() {
		...
		System.out.println("car : " + getCar()); // pricate 필드 간접 접근
	 ...
	 *  -> 물려 받은 자식에 맞게 부모의 메서드를 다시 정의야되는 경우 사용
	 *  
	 * 
	 * ** wnddy **
	 * 
	 ...
	 * 
	 * 2) 오버라이딩이 코드가 알맞게 작성되었는지
	 * 	  컴파일러에게 검사하라고 지시하는
	 * 	= 컴파일러 주석
	 * 
	 * [오버라이딩  성립 조건]
	 * 1. 접근 제한자는 같거나 넓ㅅ은 범위
	 * 2. 반환형, 매개 변수, 메서드명은 모두 동일
	 * 3. 예외처리 (구문)은 같거나 좁은 범위
	 * 
	 * (쉽게 말해서 부모랑 똑같이 쓰는게 제일 쉬움
	 * 
	 */
	
	// 컴파일러가 이 코드를 잘 쓴건지 검사
	@Override
