 ...
 * 3) 객체만 저장 가능
 * 	  (정확히는 객체 참조 주소만 저장 가능0
 * 
 * [List]
 * - 자료(Data)를 순차적으로 나열한 구조(배열 비슷)
 * 
 * - index가 존재함
...
public class ListService {
	...
	public void test1() {
		...
		// List : 부모 인터페이스
		// 			-> 객체 생성 불가 / 부모 참조 변수 가능0
//		List list = new ArrayList(); // 10칸짜리 생성
		List list = new ArrayList(3); // 3칸짜리 생성
		
		// E(Element) : 요소를 뜻하는 상식적인 글자
		// 				(특정 자료형 X, Object 타입)
		...
		list.add("아무거나");
		list.add(new Object());
		list.add(new int[4]); // 3번쨰
		
		// 4번째 크기 -> 크기 초과 요류 바생(O /x)
		//  		  --> List 크기가 자동으로 증가
		
		
		list.add(123); // int 기본 자료형은 추가 불가
					   // -> Intrger Wrapper Class로 변환되어 자정됨
...
//		menuList.add(123); // String으로 제한되서
		...
		System.out.println(before + "가 " + menuList.get(2) + "로 변경 됨");
		System.out.println(menuList);
		
		// 6. boolean contains(E e)
		// - List 내부에 e와 같은 객체가
		// 존자하면 true, 없으면 false 반환 (확이 가능)
		...
		// 존재하면 하당 index 반환
		// 존재하지 않으면 -1 반환
		System.out.println("삼겹살 : " + menuList.indexOf("삽겨살"));
		System.out.println("오겹살 : " + menuList.indexOf("오겹살"));
		
		
		// 8. E remove(int index)
		// - List에서 지정된 indenx 번째 요소를 제거
	...
	/*
	 * 왜 컬렉션 참조 변수를 부모타입으로 작성하는가?
	 * 
	 * -> 프로그래밍 과정에서 미쳐 생각지 못한 비효율적 요소를 
	 * 쉽게 개선하기 위해서
	 * 
	 * 	ex) ArrayList로 만들었는데
