...
public class StudentService {
	...
	// StudentDTO 객체를 참조하는 StudentDTO 참조 변수 5개 묶음 배열을 생성
	// 인덱스 : 0 ~ 4
	// 각 인덱스 요소 : 초기 값 null
	...
	public StudentService() {
		...
		// 0 ~ 100 사이 난수 발생
		// 1) Math.floor(Math.random() * 101) == 100
		
		// 2) Random random = new Random();
		//		random.nextInt(101); == 0 ~ 100
		
		Random random = new Random();
		for( StudentDTO std : students ) { // 향상된 포문
			...
			// random.nextInt(101) : 0 이상 101 미만 정수
	...
	public void updateScores(int index, StudentDTO other) {
		...
		// student[index] 번째에 저장된 주소 얕은 복사
		StudentDTO s = students[index];
		
		s.setHtml(other.getHtml());
		s.setCss(other.getHtml());
		s.setJs(other.getHtml());
		s.setJava(other.getHtml());
